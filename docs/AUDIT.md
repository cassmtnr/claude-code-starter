...
### H6. `writeArtifacts` silently overwrites user-customized files

- **Severity**: High
- **Category**: Runtime Failure
- **File**: `src/generator.ts:72-110`

**Description**: The merge strategy only preserves `state/task.md`. All other existing files (skills, agents, rules, commands, settings.json) are overwritten without warning. If a user has customized their `.claude/skills/testing-methodology.md` or `.claude/settings.json`, running `claude-code-starter` again destroys their changes.

The `--force` flag is documented as "Force overwrite existing .claude files" but the code already overwrites everything except `state/task.md` even WITHOUT `--force`.

**Reproduction**:
```bash
npx claude-code-starter -y  # Initial setup
echo "# My custom content" >> .claude/skills/testing-methodology.md
npx claude-code-starter -y  # Overwrites the customized file
```

**Fix**: Compare file content before writing. If the file has been modified from the generated version, skip it (like `state/task.md`) and report it as "preserved". Or add a hash/marker comment at the top of generated files to detect user modifications.
...
| H6 | High | Runtime Failure | `writeArtifacts` silently overwrites user customizations | Medium |
...
12. **Fix H6**: Implement file modification detection in `writeArtifacts` — 2-3 hours
